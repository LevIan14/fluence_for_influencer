...
class ProfileMenuInsights extends StatefulWidget {
  const ProfileMenuInsights(
      {super.key, required this.title, required this.influencer});

  final String title;
  final Influencer influencer;

  @override
  State<ProfileMenuInsights> createState() => _ProfileMenuInsightsPageState();
}

class _ProfileMenuInsightsPageState extends State<ProfileMenuInsights> {
  List<String> translatedCity = [];
    ...
    return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            FutureBuilder(
                future: translate(widget.influencer.topAudienceCity!),
                builder: (context, AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    return InstagramAudience(
                        context, widget.influencer, snapshot.data);
                  }

                  return Container();
                }),
            // InstagramAudience(context, influencer),
            InstagramInsight(widget.influencer),
          ],
        ));
  }
  ...
  Future<List<String>> translate(cities) async {
    ...
    cities.forEach((city) async {
      String translate = await AutoLocalization.translate(city,
          startingLanguage: 'en', targetLanguage: 'id');
      setState(() {
        translatedCity.add(translate);
      });
  ...
  Widget InstagramAudience(
      context, Influencer influencer, List<String> translateCity) {
    double margin = 10.0;
    ...
    cities.forEach((city) {
      citiesWidget.add(Container(
          margin: EdgeInsets.symmetric(vertical: margin / 4),
          child: Row(
            children: [
              const Icon(Ionicons.location_outline,
                  color: Color.fromARGB(129, 162, 0, 32), size: 18.0),
              Container(
  ...
  Widget InstagramInsight(Influencer influencer) {
    ...
    double reachPercent =
        ((influencer.fourWeekReach! - influencer.previousReach!) /
                influencer.previousReach!) *
            100;
    double impressionPercent =
        ((influencer.fourWeekImpressions! - influencer.previousImpressions!) /
                influencer.previousImpressions!) *
            100;
    return Container(
        margin: EdgeInsets.symmetric(vertical: margin),
        child: Column(
